    ...
    fn generate_fix(&self, issue: &Issue, source: &str, _file: &Path) -> Option<String> {
        ...
        if let Some(_suggestion) = &issue.suggestion {
            ...
            if line < source_lines.len() {
                ...
                // Try to find className attribute
                if let Some(caps) = self.class_regex.captures(line_content) {
                    if let Some(classes) = caps.get(1) {
                        ...
                        if fixed_classes != classes_str {
                            // Calculate absolute position in file (not just line-relative)
                            // Add offset from all previous lines + their newlines
                            let line_offset: usize = source_lines[..line]
                                .iter()
                                .map(|l| l.len() + 1) // +1 for newline
                                .sum();

                            // Position relative to line start
                            let relative_start = line_content.find(classes_str)?;
                            let relative_end = relative_start + classes_str.len();

                            // Convert to absolute positions
                            let start = line_offset + relative_start;
                            let end = line_offset + relative_end;

                            eprintln!("DEBUG: Generating fix for line {}: '{}' -> '{}'", line, classes_str, fixed_classes);
                            eprintln!("DEBUG: line_offset={}, start={}, end={}", line_offset, start, end);
                            ...
                            eprintln!("DEBUG: No changes needed for line {}: classes_str={}, fixed_classes={}",
                                line, classes_str, fixed_classes);
                        }
                    }
                } else {
                    eprintln!("DEBUG: No className match on line {}: '{}'", line, line_content);
                }
            } else {
                eprintln!("DEBUG: line {} >= source_lines.len() {}", line, source_lines.len());
            }
        } else {
            eprintln!("DEBUG: No suggestion for issue code '{}'", issue.code);
        }
...
impl FixRule for ImportFixRule {
    ...
    fn generate_fix(&self, issue: &Issue, source: &str, _file: &Path) -> Option<String> {
        ...
        // Calculate line offset for absolute positions
        let line_offset: usize = source_lines[..line]
            .iter()
            .map(|l| l.len() + 1) // +1 for newline
            .sum();
            ...
            if let Some(import_path) = caps.get(1) {
                ...
                for (old_prefix, new_alias) in &self.alias_replacements {
                    if path_str.starts_with(old_prefix) {
                        ...
                        let start = line_offset + import_path.start();
                        let end = line_offset + import_path.end();
