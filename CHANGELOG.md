...
### Fixed
- **Hook protocol** — Rewritten to use stdin/stdout JSON protocol (correct Claude Code command hook format). Extracts file path from `tool_input.file_path` or `tool_response.filePath`.
- **File extensions** — Added `.mjs` and `.cjs` to `VALID_EXTENSIONS`.
